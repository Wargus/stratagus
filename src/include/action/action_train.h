...
{
	friend std::unique_ptr<COrder> COrder::NewActionTrain(CUnit &trainer, CUnitType &type);
public:
	COrder_Train() : COrder(UnitAction::Train) {}

	std::unique_ptr<COrder> Clone() const override { return std::make_unique<COrder_Train>(*this); }

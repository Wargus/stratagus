...
void DrawUnitSelection(const CViewport &vp, const CUnit &unit)
{
	...
	if (Editor.Running && UnitUnderCursor == &unit && Editor.State == EditorStateType::Selecting) {
		color = ColorWhite;
...
static void DrawInformations(const CUnit &unit, const CUnitType &type, const PixelPos &screenPos)
{
	...
	snprintf(buf,
	         sizeof(buf),
	         "%d%c%c%d",
	         unit.VisCount[ThisPlayer->Index],
...
void CUnit::Draw(const CViewport &vp) const
{
	...
	if (!IsVisible && frame == UnitNotSeen) {
		ErrorPrint("FIXME: Something is wrong, unit %d not seen but drawn time %lu?.\n",
		           UnitNumber(*this),
		           GameCycle);
		return;
	}
...
**  @return Table of units to return in sorted order
**
*/
std::vector<CUnit *> FindAndSortUnits(const CViewport &vp)
{
	...
	std::vector<CUnit *> table =
		Select(minPos, maxPos, [&](const CUnit *unit) { return unit->IsVisibleInViewport(vp); });

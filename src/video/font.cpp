...
static const unsigned char codepoint_to_cp437[] = {
    //       ¡     ¢     £  ¤->°     ¥     ¦     §     ¨     ©     ª     «     ¬           ®     ¯
    0xff, 0x00, 0x9b, 0x9c, 0xf8, 0x9d, 0x00, 0x00, 0x00, 0x00, 0xa6, 0xae, 0xaa, 0x00, 0x00, 0x00, // 0xa0
    // °     ±     ²     ³     ´     µ     ¶     ·     ¸     ¹     º     »     ¼     ½     ¾     ¿
    0xf8, 0xf1, 0xfd, 0x00, 0x00, 0xe6, 0x00, 0xfa, 0x00, 0x00, 0xa7, 0xaf, 0xac, 0xab, 0x00, 0xa8, // 0xb0
    // À->à Á->á Â->â    Ã     Ä     Å     Æ     Ç  È->è     É  Ê->ê  Ë->ë  Ì->ì  Í->í  Î->î  Ï->ï
    0x85, 0xa0, 0x83, 0x00, 0x8e, 0x8f, 0x92, 0x80, 0x8a, 0x90, 0x88, 0x89, 0x8d, 0xa1, 0x8c, 0x8b, // 0xc0
    // Ð     Ñ  Ò->ò  Ó->ó  Ô->ô     Õ     Ö     ×     Ø  Ù->ù  Ú->ú  Û->û     Ü     Ý     Þ     ß
    0x00, 0xa5, 0x95, 0xa2, 0x93, 0x00, 0x99, 0x00, 0x00, 0x97, 0xa3, 0x96, 0x9a, 0x00, 0x00, 0xe1, // 0xd0
...
static unsigned char codepoint_to_codepage_index(int codepoint, const char **subst)
{
	...
	switch (codepoint) {
		case 0x192: // index for this letter in cp437 is 0x9f
			return (FontCodePage == 437) ? 0x9f : 'f';
		case 0x2502: return 0xb3;
		case 0x2591: return 0xb0;
		case 0x2592: return 0xb1;
		case 0x2593: return 0xb2;
		default:
			if (0xa0 <= codepoint && codepoint < 0xa0 + sizeof(codepoint_to_cp437)) {
				unsigned char cpChar = 0;
				// western European
				switch (FontCodePage) {
					case 437: cpChar = codepoint_to_cp437[codepoint - 0xa0]; break;
					case 1252: return codepoint;
					default: break;
				}
				if (cpChar == 0) {
					*subst = codepoint_from_0xa0_to_ascii[codepoint - 0xa0];
					return 0;
				}
				return cpChar;
			} else {
				if (0x0400 <= codepoint && codepoint < 0x0400 + sizeof(codepoint_to_cp866)) {
					// Cyrillic
					unsigned char cpChar = 0;
					switch (FontCodePage) {
						case 866: cpChar = codepoint_to_cp866[codepoint - 0x0400]; break;
						default: break;
					}
					if (cpChar == 0) {
						*subst = codepoint_from_0xa0_to_ascii[codepoint - 0x0400];
						return 0;
					}
					return cpChar;
				}
...
int CFont::Width(std::string_view text) const
{
	...
	while ((utf8 = CodepageIndexFromUTF8(text, pos, subpos))) {
		...
		if (!isformat) {
			if (this->CharWidth.size() <= utf8 - 32) {
				utf8 = 32;
			}
...
**  @param maxlen  size limit of the search. (0 means unlimited). (in char if font == nullptr else in pixels).
...
**  @param maxlen  max length of the string (0 : unlimited) (in char if font == nullptr else in pixels).
**  @param font    if specified use font->Width() instead of strlen.
**
**  @return computed value.
*/

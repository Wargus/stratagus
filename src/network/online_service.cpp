...
private:
	...
	void ensureSpace(size_t required) { assert(pos + required < MAX_MSG_SIZE); }
...
public:
	...
	std::string getStatus() const
	{
		switch (location) {
			...
			case 1: return "not in chat";
			case 2: return "in chat";
			case 3: return "in public game " + locationName;
			case 4: return "in a private game";
			case 5: return "in private game " + locationName;
	...
	std::string getProduct() const
	{
		switch (product) {
			...
			case 0x2:
			case 0x6:
			case 0xb: return "Starcraft";
			case 0x3: return "Warcraft II";
			case 0x4:
			case 0x5: return "Diablo II";
			case 0x7:
			case 0x8:
			case 0xc: return "Warcraft III";
			case 0x9:
			case 0xa: return "Diablo";
...
public:
	...
	std::string getGameSettings() const
	{
		...
		switch (settings & 0x23000) {
			case 0x01000: result += " low resources"; break;
			case 0x02000: result += " medium resources"; break;
			case 0x03000: result += " high resources"; break;
			case 0x20000: result += " random resources"; break;
		}
		switch (settings & 0x1C000) {
			case 0x04000: result += " forest"; break;
			case 0x08000: result += " winter"; break;
			case 0x0c000: result += " wasteland"; break;
			case 0x14000: result += " random"; break;
			case 0x1c000: result += " orc swamp"; break;
		}
		return result;
	}
	...
	std::string getGameStatus() const
	{
		switch (gameStatus) {
			case 0x0: return "OK";
			case 0x1: return "No such game";
			case 0x2: return "Wrong password";
			case 0x3: return "Game full";
			case 0x4: return "Game already started";
			case 0x5: return "Spawned CD-Key not allowed";
			case 0x6: return "Too many server requests";
		}
	...
	std::string getGameType() const
	{
		switch (gameSettings & 0xff) {
			...
			case 0x04: return "One vs One";
			case 0x05: return "CTF";
			case 0x06:
				switch (gameSettings & 0xffff0000) {
					case 0x00010000: return "Greed 2500 resources";
					case 0x00020000: return "Greed 5000 resources";
					case 0x00030000: return "Greed 7500 resources";
					case 0x00040000: return "Greed 10000 resources";
				}
				return "Greed";
			case 0x07:
				switch (gameSettings & 0xffff0000) {
					case 0x00010000: return "Slaughter 15 minutes";
					case 0x00020000: return "Slaughter 30 minutes";
					case 0x00030000: return "Slaughter 45 minutes";
					case 0x00040000: return "Slaughter 60 minutes";
				}
				return "Slaughter";
			case 0x08: return "Sudden Death";
			case 0x09:
				switch (gameSettings & 0xffff0000) {
					case 0x00000000: return "Ladder (Disconnect is not loss)";
					case 0x00010000: return "Ladder (Loss on Disconnect)";
				}
				return "Ladder";
			case 0x0A: return "Use Map Settings";
			case 0x0B:
			case 0x0C:
			case 0x0D:
			{
				...
				switch (gameSettings & 0xffff0000) {
					...
					case 0x00030000: sub += " (4 teams)"; break;
				}
				switch (gameSettings & 0xff) {
					case 0x0B: return "Team Melee" + sub;
			...
			case 0x0F:
				switch (gameSettings & 0xffff0000) {
					...
					case 0x00020000: return "Top vs Bottom (2v6)";
					case 0x00030000: return "Top vs Bottom (3v5)";
					case 0x00040000: return "Top vs Bottom (4v4)";
					case 0x00050000: return "Top vs Bottom (5v3)";
					case 0x00060000: return "Top vs Bottom (6v2)";
					case 0x00070000: return "Top vs Bottom (7v1)";
				}
				return "Top vs Bottom";
			case 0x10: return "Iron Man Ladder";
			default: return gameStats[5]; // just the code
		}
	}
...
public:
	...
	void startAdvertising(bool isStarted = false) override
	{
		...
		statstring << std::hex << FileChecksums
				   << ","; // cd key checksum - we use lua files checksum
		...
		switch (GameSettings.Resources) {
			case 1: game_settings |= 0x1000; break;
			case 2: game_settings |= 0x2000; break;
	...
	void reportGameResult() override
	{
		...
		for (int i = 0; i < 8; i++) {
			if (NetLocalPlayerNumber == i) {
				switch (GameResult) {
					case GameVictory: msg.serialize32NativeByteOrder(0x01); break;
					case GameDefeat: msg.serialize32NativeByteOrder(0x02); break;
					case GameDraw: msg.serialize32NativeByteOrder(0x03); break;
					default: // disconnect
		...
		switch (id) {
			case 0x05:
				...
				// (UINT32) UDP Code
				{
					uint32_t udpCode;
					memcpy(
						&udpCode, buffer + (sizeof(uint32_t) / sizeof(uint8_t)), sizeof(udpCode));
		...
		case 0x04: // recv whisper
			if (!text.empty()) {
				...
				if (text.size() > prefix.size() && text.rfind(prefix, 0) != std::string::npos) {
					int res = sscanf(text.substr(prefix.size()).c_str(),
					                 "%d.%d.%d.%d:%d",
					                 &a,
					                 &b,
					                 &c,
	...
	void doOneStep(Context &ctx) override
	{
		if (ctx.getTCPSocket().HasDataToRead(0)) {
			...
			switch (status) {
				...
				case 0x03: errMsg = "Name contains banned word(s)"; break;
				case 0x04: errMsg = "Account already exists"; break;
				case 0x05: errMsg = "Account is still being created"; break;
				case 0x06: errMsg = "Name does not contain enough alphanumeric characters"; break;
				case 0x07: errMsg = "Name contained adjacent punctuation characters"; break;

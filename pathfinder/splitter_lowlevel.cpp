...
//
/**@name splitter_lowlevel.cpp - Low level funcs for Regions.  */
//
...
{
	RegionTempStorage = new int[TheMap.Info.MapWidth * TheMap.Info.MapHeight];
}
...
int RegionTempStorageMarkObstacle(RegionId regid, int maxmark, int markvalue)
{
	...
	marked = new MapPoint[adef->TileCount];
	markednb = 0;
...
void CircularFillerInit(CircularFiller* filler, RegionId region, int startx, int starty, int value)
{
	filler->NextOne = 0;
	filler->LastOne = 0;
	filler->Points = new MapPoint[Regions[region].TileCount];

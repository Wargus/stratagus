...
int CFile::open(const char *name, long openflags)
{
	...
	if (openflags & CL_OPEN_WRITE) {
		if ((openflags & CL_WRITE_GZ) &&
				(cl_file.cl_gz = gzopen(strcat(strcpy(buf, name), ".gz"), openstring))) {
			cl_type = CLF_TYPE_GZIP;
		} else if ((cl_file.cl_plain = fopen(name, openstring))) {
			cl_type = CLF_TYPE_PLAIN;
		}
	} else {
		if (!(cl_file.cl_plain = fopen(name, openstring))) { // try plain first
			if ((cl_file.cl_gz = gzopen(strcat(strcpy(buf, name), ".gz"), "rb"))) {
				cl_type = CLF_TYPE_GZIP;
			}
		} else {
			cl_type = CLF_TYPE_PLAIN;
			// Hmm, plain worked, but nevertheless the file may be compressed!
			if (fread(buf, 2, 1, cl_file.cl_plain) == 1) {
				if (buf[0] == 0x1f) { // don't check for buf[1] == 0x8b, so that old compress also works!
					fclose(cl_file.cl_plain);
					if ((cl_file.cl_gz = gzopen(name, "rb"))) {
						cl_type = CLF_TYPE_GZIP;
					} else {
						if (!(cl_file.cl_plain = fopen(name, "rb"))) {
							cl_type = CLF_TYPE_INVALID;
						}
					}
				}
...
int CFile::close()
{
	...
	if ((tp = cl_type) != CLF_TYPE_INVALID) {
		if (tp == CLF_TYPE_PLAIN) {
			ret = fclose(cl_file.cl_plain);
		} else if (tp == CLF_TYPE_GZIP) {
			ret = gzclose(cl_file.cl_gz);
		}
	} else {
		errno = EBADF;
	}
...
int CFile::read(void *buf, size_t len)
{
	...
	if (cl_type != CLF_TYPE_INVALID) {
		if (cl_type == CLF_TYPE_PLAIN) {
			ret = fread(buf, 1, len, cl_file.cl_plain);
		} else if (cl_type == CLF_TYPE_GZIP) {
			ret = gzread(cl_file.cl_gz, buf, len);
		}
	} else {
		errno = EBADF;
	}
...
void CFile::flush()
{
	if (cl_type != CLF_TYPE_INVALID) {
		if (cl_type == CLF_TYPE_PLAIN) {
			fflush(cl_file.cl_plain);
		} else if (cl_type == CLF_TYPE_GZIP) {
			gzflush(cl_file.cl_gz, Z_SYNC_FLUSH);
		}
	} else {
		errno = EBADF;
	}
...
int CFile::printf(char *format, ...)
{
	...
	if ((tp = cl_type) != CLF_TYPE_INVALID) {
		if (tp == CLF_TYPE_PLAIN) {
			ret = fwrite(p, size, 1, cl_file.cl_plain);
		} else if (tp == CLF_TYPE_GZIP) {
			ret = gzwrite(cl_file.cl_gz, p, size);
		}
	} else {
		errno = EBADF;
	}
...
int CFile::seek(long offset, int whence)
{
	...
	if ((tp = cl_type) != CLF_TYPE_INVALID) {
		if (tp == CLF_TYPE_PLAIN) {
			ret = fseek(cl_file.cl_plain, offset, whence);
		} else if (tp == CLF_TYPE_GZIP) {
			ret = gzseek(cl_file.cl_gz, offset, whence);
		}
	} else {
		errno = EBADF;
	}
...
long CFile::tell()
{
	...
	if ((tp = cl_type) != CLF_TYPE_INVALID) {
		if (tp == CLF_TYPE_PLAIN) {
			ret = ftell(cl_file.cl_plain);
		} else if (tp == CLF_TYPE_GZIP) {
			ret = gztell(cl_file.cl_gz);
		}
	} else {
		errno = EBADF;
	}

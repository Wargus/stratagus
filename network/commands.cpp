...
static FullReplay *StartReplay(void)
{
	...
	replay->Comment1 = new_strdup("Generated by Stratagus Version " VERSION "");
	replay->Comment2 = new_strdup("Visit http://Stratagus.Org for more information");
	replay->Comment3 = new_strdup("$Id$");
	...
	if (GameSettings.NetGameType == SettingsSinglePlayerGame) {
		...
		for (i = 0; i < PlayerMax; ++i) {
			replay->Players[i].Name = new_strdup(Players[i].Name);
...
static void DeleteReplay(FullReplay *replay)
{
	...
	delete[] replay->Comment1;
	delete[] replay->Comment2;
	delete[] replay->Comment3;
	delete[] (replay->Date);
	delete[] (replay->Map);
	delete[] replay->MapPath;

	for (i = 0; i < PlayerMax; ++i) {
		delete[] replay->Players[i].Name;
	}

	log = replay->Commands;
	while (log) {
		delete[] log->UnitIdent;
		delete[] log->Action;
		delete[] log->Value;
		next = log->Next;
		delete log;
	...
	int x, int y, const CUnit *dest, const char *value, int num)
{
	...
	log->UnitIdent = (unit ? new_strdup(unit->Type->Ident) : NULL);

	log->Action = new_strdup(action);
	log->Flush = flush;
	...
	// Value given.
	//
	log->Value = (value ? new_strdup(value) : NULL);
...
static int CclLog(lua_State *l)
{
	...
	while (lua_next(l, 1)) {
		...
		if (!strcmp(value, "GameCycle")) {
			...
			log->UnitIdent = new_strdup(LuaToString(l, -1));
		} else if (!strcmp(value, "Action")) {
			log->Action = new_strdup(LuaToString(l, -1));
			...
			log->Value = new_strdup(LuaToString(l, -1));
...
static int CclReplayLog(lua_State *l)
{
	...
	while (lua_next(l, 1) != 0) {
		...
		if (!strcmp(value, "Comment1")) {
			replay->Comment1 = new_strdup(LuaToString(l, -1));
		} else if (!strcmp(value, "Comment2")) {
			replay->Comment2 = new_strdup(LuaToString(l, -1));
		} else if (!strcmp(value, "Comment3")) {
			replay->Comment3 = new_strdup(LuaToString(l, -1));
		} else if (!strcmp(value, "Date")) {
			replay->Date = new_strdup(LuaToString(l, -1));
		} else if (!strcmp(value, "Map")) {
			replay->Map = new_strdup(LuaToString(l, -1));
		} else if (!strcmp(value, "MapPath")) {
			replay->MapPath = new_strdup(LuaToString(l, -1));
			...
			for (j = 0; j < PlayerMax; ++j) {
				...
				while (lua_next(l, top) != 0) {
					...
					if (!strcmp(value, "Name")) {
						replay->Players[j].Name = new_strdup(LuaToString(l, -1));
					} else if (!strcmp(value, "Race")) {
...
static void DoNextReplay(void)
{
	...
	if (SyncRandSeed != ReplayStep->SyncRandSeed) {
		...
		if (!ReplayStep->SyncRandSeed) {
			...
			ThisPlayer->Notify(NotifyYellow, -1, -1, "No sync info for this replay !");
		} else {
